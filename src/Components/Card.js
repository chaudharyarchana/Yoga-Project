    ...
    return(
        <div className={`w-3/4 md:w-1/4 mx-auto h-96  mt-10 shadow-2xl pb-5 ${classes ? "md:ml-10 lg:ml-20": ""}`} >
            ...
            <div className="flex justify-between">
                <h1 className="ml-5 mt-1 font-bold">₹ {item.price}</h1>
                <Link href={{
            pathname:"/product",
            query: { id: item.id, title: item.title, price: item.price, img: item.img },
           
           }}>
                <button className=" bg-green-500 text-white px-4 rounded-lg mt-1 mr-5">Join</button>
                </Link>
            </div>
